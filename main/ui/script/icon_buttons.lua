...
function icon_buttons:init(druid)
	self.druid = druid
	self.buttons = {}
	setmetatable(self.buttons,{__index={len=function(len) local incr=0 for _ in pairs(len) do incr=incr+1 end return incr end}})
...
function icon_buttons:new(name, icon, color, func)
	...
	gui.set_position(new_nodes.icon_button, vmath.vector3(-16+(-40*(self.buttons:len())), 0, 0))
